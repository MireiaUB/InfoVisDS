var WaffleChart = function() {
  ...
  function drawWaffleChart(_obj) {
    ...
    legendItemdiv = legendItem.enter()
      .append("div")
    ...
    legendItemdiv
 		.append("span")
			...
			.text(function(d) { return d[$_keys[0]] +" ("+ Math.round((d.units / formattedData.length) * 100) + "%)";})
		;
		
		
    // set up the dimensions

    var width = (_obj.size * _obj.columns) + (_obj.columns * _obj.gap) - _obj.gap + _obj.size/2;
    var height = (_obj.size * _obj.rows) + (_obj.rows * _obj.gap) - _obj.gap + _obj.size/2;
    ...
    item.enter()
      ...
      .attr("cx", function(d, i) {
        ...
        return (col * (_obj.size)) + (col * _obj.gap) + _obj.size/2;
      })
      .attr("cy", function(d, i) {
        var row = i % _obj.rows;
        return (_obj.rows * (_obj.size + _obj.gap)) - ((row * _obj.size) + (row * _obj.gap)) - _obj.size /2  - _obj.gap;
      });
